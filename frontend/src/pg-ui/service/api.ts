...
export type RoleLimits = Record<string, unknown>;
export type RoleFeatures = Record<string, unknown>;
export type RoleAccess = Record<string, unknown>;
export type HWIDSettings = Record<string, unknown>;
...
function normalizeAdmin(value: unknown): AdminDetails {
  ...
  return {
    ...
    permission_overrides: row.permissionOverrides ?? row.permission_overrides,
    created_at: row.createdAt === undefined ? toNumber(row.created_at) : toNumber(row.createdAt),
    updated_at: row.updatedAt === undefined ? toNumber(row.updated_at) : toNumber(row.updatedAt),
  };
}
...
export function useCreateAdmin() {
  ...
  return useMutation({
    mutationFn: async (arg: any) => {
      const data = asRecord(arg?.data ?? arg);
...
export function useModifyAdminById() {
  ...
  return useMutation({
    mutationFn: async (arg: any) => {
      const adminId = toNumber(arg?.adminId ?? arg?.id);
      const data = asRecord(arg?.data ?? arg?.payload ?? arg);
...
export function useRemoveAdminById() {
  ...
  return useMutation({
    mutationFn: async (arg: any) => {
      const adminId = toNumber(arg?.adminId ?? arg?.id ?? arg);
...
export function useModifyRole() {
  ...
  return useMutation({
    mutationFn: async (arg: any) => {
      const roleId = toNumber(arg?.roleId ?? arg?.id);
      const data = asRecord(arg?.data ?? arg?.payload ?? arg);
...
export function useDeleteRole() {
  ...
  return useMutation({
    mutationFn: async (arg: any) => {
      const roleId = toNumber(arg?.roleId ?? arg?.id ?? arg);
...
function extractAdminIds(arg: any): number[] {
  const raw = arg?.data?.ids ?? arg?.ids ?? arg?.adminIds ?? [];
...
function useRealAdminMutation<TArg = any, TResult = any>(mutationFn: (arg: TArg) => Promise<TResult>) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn,
    onSuccess: () => {
...
export function useBulkDeleteAdmins() {
  return useRealAdminMutation(async (arg: any) => {
