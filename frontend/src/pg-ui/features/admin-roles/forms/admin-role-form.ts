...
export const FEATURE_KEYS: Array<keyof RoleFeatures> = ['can_use_reset_strategy']
...
export const adminRoleFormFromResponse = (role: AdminRoleResponse): AdminRoleFormValuesInput => ({
  ...
  disabled_when_limited: role.is_owner ? false : roleFeatureBool(role.features, 'blockLimitedAdmins', role.disabled_when_limited ?? false),
  disconnect_users_when_limited: role.is_owner ? false : roleFeatureBool(role.features, 'disconnectUsersWhenLimited', role.disconnect_users_when_limited ?? true),
  disconnect_users_when_disabled: role.is_owner ? false : roleFeatureBool(role.features, 'disconnectUsersWhenDisabled', role.disconnect_users_when_disabled ?? true),
})
