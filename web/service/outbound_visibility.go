...
func filterVisibleOutboundTraffics(traffics []*model.OutboundTraffics) []*model.OutboundTraffics {
	if len(traffics) == 0 || len(hiddenOutboundTagRules()) == 0 {
		return traffics
	}
...
func (s *OutboundService) FilterVisibleOutboundObjects(outbounds []any) []any {
	return filterVisibleOutboundObjects(outbounds)
}

func (s *OutboundService) FilterVisibleOutboundTags(tags []string) []string {
	return filterVisibleOutboundTags(tags)
}

func (s *OutboundService) FilterVisibleOutboundsInXraySetting(xraySetting string) string {
...
func (s *OutboundService) MergeHiddenOutboundsIntoXraySetting(submittedSetting string, currentSetting string) (string, error) {
